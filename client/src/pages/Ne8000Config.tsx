...
import { Radio, Save, Plus, Trash2, Server, Network } from "lucide-react";
      ...
      <div className="rounded-xl border" style={{ background: "oklch(0.13 0.012 260)", borderColor: "oklch(0.22 0.015 260)" }}>
        ...
        <div className="divide-y" style={{ borderColor: "oklch(0.18 0.012 260)" }}>
          {!operators?.length ? (
            ...
            <div key={op.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
              ...
              {isAdmin && <Button variant="ghost" size="icon" className="w-7 h-7 text-muted-foreground hover:text-red-400 hover:bg-red-500/10" onClick={() => { if (confirm("Remover operadora?")) deleteOp.mutate({ id: op.id }); }}><Trash2 className="w-3.5 h-3.5" /></Button>}
            </div>
          ))}
        </div>
      </div>
