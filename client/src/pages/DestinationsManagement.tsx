...
export default function DestinationsManagement() {
  ...
  const createMutation = trpc.destinations.create.useMutation({
    onSuccess: () => { toast.success("Destino adicionado"); utils.destinations.list.invalidate(); setForm({ name: "", host: "" }); setShowForm(false); },
      ...
      {showForm && isAdmin && selectedOp && selectedOp !== "all" && (
        <div className="rounded-xl border p-5 space-y-4" style={{ background: "oklch(0.11 0.01 260)", borderColor: "oklch(0.22 0.015 260)" }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label className="text-xs text-muted-foreground">Nome / Identificação</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="AWS São Paulo" className="h-9 bg-input border-border text-sm" /></div>
            <div className="space-y-1.5"><Label className="text-xs text-muted-foreground">IP ou Hostname</Label><Input value={form.host} onChange={e => setForm(f => ({ ...f, host: e.target.value }))} placeholder="8.8.8.8" className="h-9 bg-input border-border text-sm" /></div>
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => createMutation.mutate({ operatorId: Number(selectedOp), ...form })} disabled={createMutation.isPending || !form.name || !form.host} className="h-8 text-xs gap-1.5"><Plus className="w-3 h-3" />{createMutation.isPending ? "Adicionando..." : "Adicionar"}</Button>
        ...
        <div className="divide-y" style={{ borderColor: "oklch(0.18 0.012 260)" }}>
          {!destinations?.length ? (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">Selecione uma operadora e adicione destinos para monitorar</p>
            </div>
            ...
            return (
              <div key={dest.id} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "oklch(0.55 0.20 255 / 0.1)" }}><Target className="w-4 h-4 text-primary" /></div>
                ...
                {isAdmin && <Button variant="ghost" size="icon" className="w-7 h-7 text-muted-foreground hover:text-red-400 hover:bg-red-500/10" onClick={() => { if (confirm("Remover destino?")) deleteMutation.mutate({ id: dest.id }); }}><Trash2 className="w-3.5 h-3.5" /></Button>}
              </div>
            );
          })}
        </div>
